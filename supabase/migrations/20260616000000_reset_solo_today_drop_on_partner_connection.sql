...
begin
  ...
  then
    ...
    where dd.couple_id = new.id
      ...
      and (
        old.partner_type is distinct from new.partner_type
        or dd.created_at < coalesce(new.connected_at, now())
...
drop trigger if exists reset_today_drop_on_partner_type_change on public.couples;
create trigger reset_today_drop_on_partner_type_change
after update of partner_type, status, connected_at on public.couples
for each row
execute function public.reset_today_drop_on_partner_type_change();
