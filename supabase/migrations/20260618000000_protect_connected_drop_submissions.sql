create or replace function public.has_connected_drop_submission(
  p_drop_id uuid,
...
as $$
  select exists (
    ...
    where ds.drop_id = p_drop_id
      and p_connected_at is not null
      and ds.submitted_at >= p_connected_at
  );
$$;
...
begin
  ...
  then
    ...
    where dd.couple_id = new.id
      and not public.has_connected_drop_submission(dd.id, new.connected_at)
      and (
        (
          old.status = 'pending'
...
begin
  ...
  if v_drop.id is not null then
    ...
    if (
      ...
      or public.has_connected_drop_submission(v_drop.id, v_couple.connected_at)
    then
    ...
    where id = v_drop.id
      and not public.has_connected_drop_submission(v_drop.id, v_couple.connected_at);

    if public.has_connected_drop_submission(v_drop.id, v_couple.connected_at) then
      return jsonb_build_object(
        'daily_drop', to_jsonb(v_drop),
        'mission', to_jsonb(v_mission),
        'couple', to_jsonb(v_couple),
